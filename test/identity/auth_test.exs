defmodule Identity.AuthTest do
  ...
  @invalid_attrs %{
    email: nil, password: nil, firstname: nil, lastname: nil
  }
  ...
  describe "auth" do
    ...
    test "login/2 logs user in when credentials are correct" do
      user = user_fixture()
      assert {:ok, %User{} = _user, access_token, refresh_token} = Auth.login(user.email, user.password)
    ...
    test "renew_access/1 will not renew revoked token" do
      user = user_fixture()
      assert {:ok, %User{} = _user, _access, refresh_token} = Auth.login(user.email, user.password)
    ...
    test "revoke_refresh_token/1 will revoke existing/valid refresh token" do
      user = user_fixture()
      assert {:ok, %User{} = _user, _access_token, refresh_token} = Auth.login(user.email, user.password)
